...
float shadowCalculation(vec4 fragPosLightSpace, vec3 normal) {
	...
	float bias = 0.00001;//max(0.05 * (1.0 - dot(normal, light_dir)), 0.005);
...
void main() {
    ...
    float ao = texture(texture_ambient_occlusion, texcoords).x;;

    vec3 N = vec3(texture(texture_normal, texcoords));
    ...
    float s = shadowCalculation(light_space_pos, N);
	color -= vec3(s, s, s);
